...
<table table class="table">
  ...
  ?>
      <tr align="center">
        ...
        <td>
          <a href="bekukan.php?id=<?php echo $d['id_PO']; ?>">BEKUKAN</a>
        </td>
      </tr>
      <?php 
        }
...
<table table class="table">
  ...
  ?>
      <tr align="center">
        ...
        <td><?php echo $d['no_hp']; ?></td>
        <td>
          <a href="bekukan.php?id=<?php echo $d['no_ktp']; ?>">BEKUKAN</a>
        </td>
      </tr>
      <?php 
        }
...
<?php
    ...
    if(isset($_GET['id'])){
        $id = $_GET['id'];
        $result = mysqli_query($host, "Select * from bekuakun where idBeku='$id'");
        if(mysqli_num_rows($result) > 0){
            echo "<script>alert('Akun $id sudah di bekukan');</script>";
        } else {
            $query = "INSERT INTO `bekuakun`(`idBeku`) VALUES ('$id')";
            mysqli_query($host, $query);
            echo "<script>location='bekukan.php';</script>";
        }
    }
?>